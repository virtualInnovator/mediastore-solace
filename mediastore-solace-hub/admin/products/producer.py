...
TOPIC_PREFIX_LOCAL = "solace/mediastore/local/"
TOPIC_PREFIX_REMOTE = "solace/mediastore/remote/"
...
def publish(method, body):
    ...
    broker_props = {
        ...
        "solace.messaging.service.vpn-name": os.environ.get('SOLACE_VPN') or "mediastore-hub-service",
        "solace.messaging.authentication.scheme.basic.username": os.environ.get(
            'SOLACE_USERNAME') or "solace-cloud-client",
        "solace.messaging.authentication.scheme.basic.password": os.environ.get('SOLACE_PASSWORD') or "dv5k30n4466veudtmr6nge929f"
    }
    ...
    topic = Topic.of(TOPIC_PREFIX_LOCAL + f'main')
...
def publish_to_remote(method, body):
    # Broker Config
    broker_props = {
        "solace.messaging.transport.host": os.environ.get(
            'SOLACE_HOST') or "tcps://SOLACE_HOST:55443",
    ...
    # Prepare the destination topic
    topic = Topic.of(TOPIC_PREFIX_REMOTE + f'main')
